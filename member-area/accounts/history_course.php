...
$Course_ID =base64_decode($_GET["Course_ID"]);
			$result = mysql_query("SELECT * FROM `class_history` WHERE date_admin = '$con' AND course_id = '$Course_ID' AND monitor_id > '1'");
...
function uuu($con){
$Course_ID =base64_decode($_GET["Course_ID"]);
			$result = mysql_query("SELECT `class_history`.*,`monitor`.`mnt_name`,`profile`.`teacher_name` FROM `class_history`,`monitor`,`profile` WHERE class_history.monitor_id=monitor.mnt_id AND class_history.teacher_id=profile.teacher_id HAVING date_admin = '$con' AND course_id = '$Course_ID' AND monitor_id > '1'");
...
$result1 = mysql_query("SELECT * FROM invoice WHERE status = 1 and parent_id ='$tt'");
$counter = 0;
if (!$result) 
	{
    die("Please contact Admin");
	}
$numberOfRowsot1 = MYSQL_NUMROWS($result1);
If ($numberOfRowsot == 0) 
	{
	echo '0';
	}
...
if (!$result) 
		...
		die("Query to show fields from table failed");
			...
			$course = MYSQL_RESULT($result,$i,"course_yrSec");
				...
				echo $course;				
				?></b></font> for <?php echo date('F'); ?>-<?php echo $sy; ?></h3>
