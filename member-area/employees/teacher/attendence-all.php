				...
				do {  ?>
                    ...
                    <?php } while ($row = mysql_fetch_assoc($result)); ?>
												...
												<div class="form-group">
															<label class="control-label col-md-3">
															<strong>Select 
															Additional Lesson</strong></label>
															<div class="col-md-9">
															<select class="form-control" name="aalesson"  id="aalesson" onchange="javascript: return optionList9_SelectedIndex()">
                    <?php 
                // source 1: http://www.dmxzone.com/showDetail.asp?NewsId=5102&TypeId=25
			  	// source 2: http://localhost/phpmyadmin/index.php?db=mydbase&token=651c0063e511c381c9c82ce1fe9b6854
				$result = mysql_query("SELECT * FROM lesson_pages WHERE dept_id = $adept OR dept_id = 100 ORDER BY lesson_id");			  	
				do {  ?>
                    <option value="<?php echo $row['lesson_id'];?>"><?php echo $row['lesson_name'];?></option>
                    <?php } while ($row = mysql_fetch_assoc($result)); ?>
                    </select>															
