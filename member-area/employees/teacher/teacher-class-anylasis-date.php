...
function class_monitor($var2, $var3, $var4){
    $sql2="SELECT SEC_TO_TIME(SUM(TIME_TO_SEC(`duration`))) AS total FROM class_history WHERE teacher_id = '$var2' AND status != 11 AND monitor_id = '$var3' AND date_teacher = '$var4' AND re_status = 1";
...
function class_monitor_num($var2, $var3, $var4){
    $result = mysql_query("SELECT history_id FROM class_history WHERE teacher_id = '$var2' AND monitor_id = '$var3' AND status != 11  AND date_teacher = '$var4' AND re_status = 1");
if (!$result) 
	{
    die("Query to show");
	}
...
function class_paid($var2, $a, $var4)
{
    $date_start =$_REQUEST['date1'];
$date_end =$_REQUEST['date2'];
    $sql2="SELECT SEC_TO_TIME(SUM(TIME_TO_SEC(`duration`))) AS total FROM class_history WHERE teacher_id = '$var2' AND status != 11 AND re_status != 2 AND date_teacher = '$var4' AND (monitor_id = 4 OR monitor_id = 9  OR monitor_id = 21)";
$res2 = mysql_query($sql2);
$totalRow = mysql_fetch_array($res2);
$total = $totalRow['total'];
if ($total != 0){echo $total;}
...
<a onclick="goBack()" class="btn default">
						...
						<div class="portlet-body">
							<div id="mytable" class="table-responsive">
								...
								<?php
// sending query
   $result = mysql_query("SELECT * FROM `class_history` WHERE teacher_id = '$tech_id' AND status != 11 AND date_teacher >= '$date_start' AND date_teacher <= '$date_end' GROUP BY date_teacher;");
   $counter = 0;
if (!$result) 
	{
    die("Query to show");
