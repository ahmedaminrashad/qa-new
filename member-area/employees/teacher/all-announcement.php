...
function cun_ann()
  	{$sy = date('Y-m-d');
  		$result = mysql_query("SELECT * FROM announcement WHERE (ann_cat = 8 or ann_cat = 17) and ann_date <= '$sy' AND ann_end >= '$sy'");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
...
function all_ann()
  	{$sy = date('Y-m-d');
  		$result = mysql_query("SELECT * FROM announcement where ann_cat = 8 or ann_cat = 17");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
								...
								<div class="todo-project-list">
									<ul class="nav nav-pills nav-stacked">
										<li class="active">
											...
											<span class="badge badge-success"> <?php echo cun_ann(); ?> </span> ACTIVE </a>
										</li>
										<li>
											<a href="all-announcement">
											<span class="badge badge-success badge-active"> <?php echo all_ann(); ?> </span> ALL </a>
										</li>
									</ul>
								</div>
							</div>
