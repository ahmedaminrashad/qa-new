...
include("../includes/teacher_rights.php"); include("header.php");
 ...
 function LeaveApproved($t, $s)
  	{
  		$result = mysql_query("SELECT * FROM leave_app WHERE teacher_id = '$t' AND status = '$s'");
$counter = 0;
if (!$result) 
	{
...
function LeaveApproved1($t)
  	{
  		$result = mysql_query("SELECT * FROM leave_app WHERE teacher_id = '$t' AND (status = '1' OR status = '2' OR status = '3' OR status = '4')");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
								...
								<div class="row">
									<div class="">
										<div class="scroller" style="max-height: 800px;" data-always-visible="0" data-rail-visible="0" data-handle-color="#dae3e7">
											...
											<?php 
// sending query
$result = mysql_query("SELECT * FROM leave_app WHERE teacher_id = $ID AND (status = 1 OR status = 2 OR status = 3 OR status = 4) ORDER BY leave_id DESC;");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
...
?>
													...
													<div class="todo-tasklist-item-title">
														<font color="#A80707">FROM: <?php echo $sdate; ?>  --  TO: <?php echo $tdate; ?></font>
													</div>
