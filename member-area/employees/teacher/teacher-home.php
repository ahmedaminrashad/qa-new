...
$today = date("Y-m-d", strtotime('now +1 hour'));
$datep = date('Y-m-d');
					...
					<div class="portlet light">
						...
						<div class="portlet-body">
							<div id="mytable" class="table-responsive">
								...
								<?php if ($mnp == 3 && $active_msg != 1) {echo "<a onclick='return target_link(this)' href='notification_new2?Mid=1?&history_id=$history_id&Sid=".$student_id."&Tid=".$teacher_id."&Pid=".$parent_id."&y=".time()."'><button type='button' style='text-transform: none;   background: #359dd2; ' class='btn  btn-xs' title='Send a notice to the student, telling him to enter the room.'>Notification</button></a>";} ?>
								<?php if ($mnp == 3 && $active_msg == 1 && $isSendNotification != 1) {echo "<a  onclick='return target_link(this)'href='notification_new2?Mid=2?&history_id=$history_id&Sid=".$student_id."&Tid=".$teacher_id."&Pid=".$parent_id."&y=".time()."'><button type='button' style ='text-transform: none;  background: #006780; '  class='btn  btn-xs' title='If Student not there in the class click to send notification to student.'>Reminder</button></a>";} ?>
								<?php
							...
							//	elseif ($mnp == 3 && $dept_id == 3) {echo "<a href='attendence-hifz?history_id=".$history_id."&dept_id=".$dept_id."&adept_id=".$adept_id."'><button type='button' style='width: 75px;' class='btn btn-danger btn-xs' title='If you have taken the class and given the lesson to student, please click on END button to put class history.'>End</button></a>";} ?>
								...
								<?php if ($mnp == 3) {echo "<a onclick='return target_link(this)' href='notification_new2?Mid=4?&history_id=$history_id&Sid=".$student_id."&Tid=".$teacher_id."&Pid=".$parent_id."&y=".time()."'><button type='button' style='width: 75px;text-transform: none;' class='btn red btn-xs' title='If you waited for the whole class time and student did not join the meetig room, mark him Absent.'>Absent</button></a>";} else {echo "";} ?>
