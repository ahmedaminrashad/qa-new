		...
		<div class="container">
			...
			<div class="row">
				<div class="col-md-12">
					...
					<h4><?php 
$result = mysql_query("SELECT * FROM voucher Where MONTH(date) = $month and YEAR(date) = $year");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
...
<a class="btn default" data-target="#full-width" data-toggle="modal">
						...
						<div class="portlet-body">
							<div id="mytable" class="table-responsive">
								...
								<?php 
// sending query
$result = mysql_query("SELECT `voucher`.*, `bank_account`.`bank_name` FROM `voucher`,`bank_account` WHERE voucher.bank_id=bank_account.bank_id HAVING MONTH(date) = $month and YEAR(date) = $year ORDER BY date");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
