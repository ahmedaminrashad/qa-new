...
echo $numberOfRows; ?>)</b></a>
						...
						<div class="portlet-body">
							<div id="mytable" class="table-responsive">
								...
								<?php 
$result = mysql_query("SELECT `account`.*, `currency`.`currency_name`, `time_zone`.* FROM `account`,`currency`,`time_zone` WHERE account.currency_id=currency.currency_id and account.timezone=time_zone.tz_id HAVING active = 1 OR active = 5 OR active = 11 OR active = 7 ORDER BY parent_id DESC;");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
