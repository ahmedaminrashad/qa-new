...
?>)</a>
						...
						<div class="portlet-body">
							<div id="mytable" class="table-responsive">
								...
								<?php 
// sending query
$result = mysql_query("SELECT `invoice`.*, `month`.`month_name`, `statusp`.`name`, `country`.`c_name` FROM `invoice`,`month`,`statusp`,`country` WHERE invoice.mon_id=month.month_id and invoice.status=statusp.s_id and invoice.c_id=country.c_id HAVING mon_id = $m and status = $s and y_id = $y and currency_id = $cuid ORDER BY py_id DESC");
if (!$result) 
	{
    die("Query to show fields from table failed");
	}
...
?>
								...
								<tbody>
								<tr>
								<td><a href="parent-accounts-profile-invoice?parent_id=<?php echo base64_encode($prid); ?>"><button type="button" class="btn red btn-xs"><?php echo abc2("$prid"); ?></button></a></td>
								<td>
