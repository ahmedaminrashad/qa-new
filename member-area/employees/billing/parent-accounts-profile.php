...
$tnumberOfRows = MYSQL_NUMROWS($result);
If ($tnumberOfRows == 0){
			echo '<button type="button" class="btn red btn-xs"><i class="fa fa-ban"></i></button>';
			}
		else if ($tnumberOfRows > 0) 
			{echo '';}
			}
...
$tnumberOfRows = MYSQL_NUMROWS($result);
If ($tnumberOfRows == 0){
			echo '<button type="button" class="btn blue btn-xs"><i class="fa fa-user"></i></button>';
			}
		else if ($tnumberOfRows > 0) 
			{echo '';}
			}
...
If ($tnumberOfRows == 0){
   ...
   $pnid =base64_decode($_GET["parent_id"]);
	$result = mysql_query("SELECT `account`.*, `currency`.`currency_name`, `country`.`c_a`, `manager`.`username`, `time_zone`.* FROM `account`,`currency`, `country`, `manager`, `time_zone` where account.currency_id=currency.currency_id and account.c_id=country.c_id and account.m_id=manager.user_id and account.timezone=time_zone.tz_id HAVING parent_id = '$pnid'");
	if (!$result) 
		{
		die("Query to show fields from table failed");
		}
								...
								<div class="row">
									<div class="col-md-3">
										<ul class="list-unstyled profile-nav">
											...
											<li>
												<a href="parent-accounts-student?p_id=<?php echo base64_encode($pnid); ?>">
												<i class="fa fa-child"></i> List of Students</a>
											</li>
											<li>
												<a href="save-student?t_id=<?php echo $pid; ?>&cp_id=<?php echo $pc11; ?>&man_id=<?php echo $pman; ?>&trial_id=<?php echo $active_s; ?>&time_id=<?php echo $ptz; ?>">
									...
									<div class="col-md-9">
										<div class="row">
											...
											<ul class="list-inline">
											<li>
														<a href="save-student?t_id=<?php echo $pid; ?>&cp_id=<?php echo $pc11; ?>&man_id=<?php echo $pman; ?>&trial_id=<?php echo $active_s; ?>&time_id=<?php echo $ptz; ?>"><button type="button" class="btn blue btn-xs"><i class="fa fa-plus"></i> Add New Student</button></a>
													</li>
													<li>
													...
													<li>
														<a data-target="#task" data-toggle="modal"><button type="button" class="btn green btn-xs"><i class="fa fa-comments"></i> Add Task</button></a>
													</li>
													</ul>
											</div>
											<!--end col-md-8-->
...
?>
														...
														<tr>
															<td>
																<font color="<?php echo $bgcolor; ?>"><?php echo $mon; ?></a></font>															</td>
															<td class="hidden-xs">
															...
															<td>
																<font color="<?php echo $bgcolor; ?>"><?php if($user_id<=12) {echo $cuan;} else {echo "Nil";} ?> <?php if($user_id<=12) {echo $fe;} else {echo "Nil";} ?>/-</font>
															</td>
															<td>
																<a href="paid.php?payment_id=<?php echo $pidy; ?>" onclick="return confirm('<?php echo "You are about to mark this invoice paid. Please confirm your action."; ?>');"><?php if ($s == 1){echo '<button type="button" class="btn red btn-xs">Mark Paid</button>'; }?></a><?php if ($s == 2){ echo '<button type="button" class="btn green btn-xs">Paid</button>'; } ?>
															</td>
															<td><a href="edit-invoice?pT=<?php echo base64_encode($pidy); ?>&link=<?php echo base64_encode($link); ?>"><button type="button" class="btn yellow btn-xs">Edit</button></a></td>
															<td><a href="delete-invoice?pT=<?php echo $pidy; ?>"><button type="button" class="btn red btn-xs">Delete</button></a></td>
														</tr>
														<?php
														$i++;		
		}
...
?>
								...
								<td>
									 <a href="history_course_<?php echo $dept_id; ?>?Course_ID=<?php echo base64_encode($Course_ID); ?>"><?php echo ++$counter; ?></a>
								</td>
								<td>
								...
								<td><span class="label label-sm label-success" title="Taken Regular"><?php echo classes("$Course_ID", "$ptea", "$mm_id", "$yy_id", "1"); ?></span>-<span class="label label-sm label-info" title="Leave"><?php echo classes("$Course_ID", "$ptea", "$mm_id", "$yy_id", "5"); ?></span>-<span class="label label-sm label-danger" title="Absent"><?php echo classes("$Course_ID", "$ptea", "$mm_id", "$yy_id", "4"); ?></span>-<span class="label label-sm label-warning" title="Pending"><?php echo classes("$Course_ID", "$ptea", "$mm_id", "$yy_id", "9"); ?></span>-<span class="label label-sm label-success" title="Classes/Week"><?php echo schedule_classes($Course_ID); ?></span>
</td>
								<td><a href="edit-student-account?Course_ID=<?php echo $Course_ID; ?>&link=<?php echo $link; ?>"><button type="button" class="btn yellow btn-xs" title="Edit Student Data"><i class="fa fa-edit"></i></button></a></td>								
...
If ($numberOfRows == 0) 
	{
	echo '<font color="#0DB5C0">No Note Found</font>';
	}
else if ($numberOfRows > 0) 
	{
	$i=0;
...
?>
					...
					<div class="modal fade" id="note" tabindex="-1" role="dialog" aria-hidden="true">
										...
										<form action="add-chat-comment" method="post" class="form-horizontal form-row-seperated">
											<div class="form-body">
												...
												<div class="form-group">
															...
															<div class="col-md-4">
															<select required class="form-control input-circle" name="pman"  id="pman" onchange="javascript: return optionList49_SelectedIndex()">
                      							<option value="7">General</option>
												<option value="11">Mr. Sajid</option>
												<option value="12">Mr. Abdul Hafiz</option>
              									</select>
              												</div>
												</div>
												<div class="form-group">
												...
												<input type="hidden" value="<?php echo $user_id; ?>" name="u_id" id="u_id" class="form-control input-circle"></input>
								...
								<div class="portlet box green">
									<div class="portlet-title">
										<div class="caption">
											<i class="fa fa-plus"></i>Add Task for <?php echo $pname; ?>
										</div>
										<div class="tools">
											<a href="javascript:;" class="collapse">
											</a>
									...
									<div class="portlet-body form">
										<!-- BEGIN FORM-->
										<form action="add-new-task" method="post" class="form-horizontal form-row-seperated">
											<div class="form-body">
												<div class="form-group">
															<label class="control-label col-md-3">
															<strong>Parent Name</strong></label>
												...
												<option value="Mr. Sajid">Mr. Sajid</option>
												<option value="Mr. Abdul Hafeez">Mr. Abdul Hafeez</option>
              									</select>
              												</div>
												</div>
												<div class="form-group">
															...
															<div class="col-md-4">
															<select required class="form-control input-circle" name="stu"  id="stu" onchange="javascript: return optionList49_SelectedIndex()">
												...
												<input type="hidden" value="<?php echo $pname; ?>" name="p_id" id="p_id" class="form-control input-circle"></input>
												<input type="hidden" value="<?php echo $sy11; ?>" name="time" id="time" class="form-control input-circle"></input>
												<input type="hidden" value="<?php echo $pid; ?>" name="ppid" id="ppid" class="form-control input-circle"></input>
											<div class="form-actions">
												<div class="row">
													<div class="col-md-offset-3 col-md-9">
														<button type="submit" name="cmdSubmit" class="btn btn-circle blue">
