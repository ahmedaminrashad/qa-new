...
function abc($d, $pTw)
  {
			...
			$result = mysql_query("SELECT `sched`.*,`course`.`course_yrSec` FROM `sched`,`course` WHERE sched.course_id=course.course_id HAVING day_id= '$d' AND teacher_id='$pTw' AND time_s_id= '$t'");
...
function color($d, $pTw){
$t = $_POST["pstime"];
			$result = mysql_query("SELECT * FROM sched HAVING day_id = $d and teacher_id = $pTw and time_s_id = $t
  			");
			//HAVING course_id='$pCourse'
		if (!$result) 
			{
...
if (!$result1) 
		...
		echo "Avaiabliblty of Class at <br>Studnet Time: <font color='#44B6AE'> <b>$teacher</b></font> <br>Teacher Time: <font color='#44B6AE'> <b>$teacher</b></font> <br>Time-Zone: <font color='#44B6AE'> <b>Africa/Cairo</b></font>"; ?>
</h5>
...
$t = $_POST["pstime"];
$gd = $_POST["pgender"];
$result = mysql_query("SELECT * FROM profile WHERE (st1 <= $t AND et1 >= $t) OR (st2 <= $t AND et2 >= $t) OR (st3 <= $t AND et3 >= $t) HAVING (cat_id = 8 or teacher_rights = 1) and inout_id = 1 AND active =1 AND schedule_status = 1 AND training = 1 AND g_id = $gd");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
...
?>
								...
								<td bgcolor="<?php echo color('1',"$pT"); ?>">
									 <font color="#FFFFFF"><?php echo abc('1',"$pT"); ?></font>
								</td>
								<td bgcolor="<?php echo color('2',"$pT"); ?>">
									 <font color="#FFFFFF"><?php echo abc('2',"$pT"); ?></font>
								</td>
								<td bgcolor="<?php echo color('3',"$pT"); ?>">
									 <font color="#FFFFFF"><?php echo abc('3',"$pT"); ?></font>
								</td>
								<td bgcolor="<?php echo color('4',"$pT"); ?>">
									 <font color="#FFFFFF"><?php echo abc('4',"$pT"); ?></font>
								</td>
								<td bgcolor="<?php echo color('5',"$pT"); ?>">
									 <font color="#FFFFFF"><?php echo abc('5',"$pT"); ?></font>
								</td>
								<td bgcolor="<?php echo color('6',"$pT"); ?>">
									 <font color="#FFFFFF"><?php echo abc('6',"$pT"); ?></font>
								</td>
								<td bgcolor="<?php echo color('7',"$pT"); ?>">
									 <font color="#FFFFFF"><?php echo abc('7',"$pT"); ?></font>
								</td>
							</tr>
							<?php 	
		$i++;		
