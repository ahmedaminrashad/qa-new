...
If ($tnumberOfRows1 == 0){
		...
		else if ($tnumberOfRows1 > 0) 
			{
					$hidden_tt = MYSQL_RESULT($result1,$i,"mnt_name");
			  		echo '<span class="label label-sm label-danger">'.$hidden_tt.'</span>';
			}
}
?>
...
$new_date = date('l jS F-Y', $old_date_timestamp); echo '<span class="label label label-success">'.$new_date.'</span>'; ?> <span class="label label bg-blue-hoki">All Classes</span></h2>
						<table class="table table-hover">
								...
								<?php
// sending query
   $result = mysql_query("SELECT * FROM `class_resched` ORDER BY atime_s_id;
  			");
if (!$result) 
	{
    die("Query to show fields from table failed");
...
If ($numberOfRows == 0) 
	...
	while($row = mysql_fetch_array($result))
		{		
			...
			$uni_id = MYSQL_RESULT($result,$i,"uni");
			...
			$his_ledate = MYSQL_RESULT($result,$i,"le_date_admin");
			$his_ori = MYSQL_RESULT($result,$i,"ori_status");
			$his_cur = MYSQL_RESULT($result,$i,"cur_status");
?>
							</tr>
								</thead>
								<tbody>
								<tr bgcolor="<?php if ($his_cur == 1) { echo '#F2F5A9'; } elseif ($his_cur == 2) { echo '#C3C3FA'; } else { echo '#ffffff'; } ?>">
								<td>
									 <?php echo date('d M Y (D)',strtotime($his_ledate)); ?>
								</td>
								<td>
									 <?php echo date('d M Y (D)',strtotime($his_date)); ?>
								</td>
								<td>
								...
								<td><?php echo pagelevel("$his_ori"); ?></td>
								<td><a href="approve-reschedule?his_id=<?php echo $sched_id; ?>&original_id=<?php echo $uni_id; ?>"><span class="label label-sm label-success">Yes</span></a></td>
								<td><?php if ($his_cur == 1) { echo '<a href="not-reschedule?his_id='.$sched_id.'&original_id='.$uni_id.'"><span class="label label-sm label-danger">No</span></a>'; } elseif ($his_cur == 2) { echo '<a href="not-advance?his_id='.$sched_id.'&original_id='.$uni_id.'"><span class="label label-sm label-danger">No</span></a>'; }?></td>
