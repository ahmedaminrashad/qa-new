...
?>
														...
														<tr>
															...
															<td class="hidden-xs">
																 <a class="invoicelink" href="#invoice" data-toggle="modal" data-target="#invoice" data-id="<?php echo $pidy; ?>" data-amu="<?php echo $fe+$fe_add; ?>" data-acc="103455664"><font color="<?php echo $bgcolor; ?>"><?php echo substr($mon, 0, 3); ?></font></a>
															</td>
															<td>
															...
															<td><a href="mail-direct-link?invoice_no=<?php echo $pidy; ?>&account=103455664&month=<?php echo $mon; ?>&year=<?php echo $ayear; ?>&cur=<?php echo $cuan; ?>&amount=<?php echo $fe+$fe_add; ?>&parent=<?php echo $prid; ?>&link=<?php echo $link; ?>"><button type="button" title="Email Direct Payment Link" class="btn yellow btn-xs"><i class="fa fa-envelope"></i></button></a></td>
															<td><a href="mail-direct-link-recurring?invoice_no=<?php echo $pidy; ?>&account=103455664&month=<?php echo $mon; ?>&year=<?php echo $ayear; ?>&cur=<?php echo $cuan; ?>&amount=<?php echo $fe+$fe_add; ?>&parent=<?php echo $prid; ?>&link=<?php echo $link; ?>"><button type="button" title="Email Recurring Payment Link" class="btn green btn-xs"><i class="fa fa-envelope"></i><i class="fa fa-star-o"></i></button></a></td>
														</tr>
														<?php
														$i++;		
		}
...
?>
								...
								<td>
									<?php if ($pnat == 1) { echo '<a href="add-schedule-local-profile?Course_ID='.$Course_ID.'&tech='.$teacher_id.'&studentName='.$course_yrSec.'&link='.$link.'&nod='.$nod.'"><button type="button" class="btn blue btn-xs" title="Add Schedule"><i class="fa fa-history"></i></button></a>'; } ?>
								</td>
								<td>
...
?>
								...
								<tr class="warning">
									...
									<td><?php echo $cu; ?> <?php $sql = "select sum(Fee) from course WHERE parent_id = '$pid' AND nature = '1'";
$q = mysql_query($sql);
$row = mysql_fetch_array($q);
$feeTotal = $row[0];
echo $feeTotal;
?></td>
								<td><a class="discount" href="#invoice" data-toggle="modal" data-target="#invoice" data-id="<?php echo $pid; ?>" data-dis="<?php echo $pdiscount; ?>"><?php echo ''.$pdiscount.'%'; ?></td>
								<td colspan="3"><?php echo $cu; ?> <?php $sql = "select sum(Fee) from course WHERE parent_id = '$pid' AND nature = '1'";
$q = mysql_query($sql);
$row = mysql_fetch_array($q);
$feeTotal = $row[0];
$fdis = ($feeTotal*$pdiscount)/100;
echo $feeTotal-$fdis;
?></td>
								</tr>
								</tbody>
...
$('.invoicelink').click(function(){
    ...
    $.ajax({url:"invoice-link-details.php?famID="+famID+"&famAmu="+famAmu+"&famAcc="+famAcc,cache:false,success:function(result){
        $(".modal-content").html(result);
    }});
});
</script>
