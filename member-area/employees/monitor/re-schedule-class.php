...
$admin_date = date('Y-m-d',strtotime("".$date1." ".$b." days"));
			...
			header(
			"Location: save-re-schedule-class.php?teacher=".$teacherid."&student=".$studentid."&t_date=".$teacher_date."&s_date=".$student_date."&a_date=".$admin_date."&t_time=".$teacher_time."&s_time=".$student_time."&a_time=".$admin_time."&history=".$history."&link=".$link."&dept=".$dept."&a_dept=".$adept."&parent_id=".$parent."&admindate=".$add."&teacherdate=".$tdd."&studentdate=".$sdd."");
	}
?>
<?php
...
If ($tnumberOfRows == 0){
	...
	$i++;	 
											...
											<div class="form-group">
															...
															<input readonly type="text" class="form-control" value="<?php $sys = $_REQUEST['Course_ID']; $result = mysql_query("SELECT * FROM course Where course_id = $sys");
if (!$result) 
	{
    die("Query to show fields from table failed");
	}
...
If ($tnumberOfRows == 0){
			...
			while ($i<$tnumberOfRows)
				{			
					$hidden_pt11 = MYSQL_RESULT($result,$i,"course_yrSec");
					$hidden_pday11 = MYSQL_RESULT($result,$i,"course_id");
	...
	$i++;	 
				...
				do {  ?>
                    ...
                    <?php } while ($row = mysql_fetch_assoc($result)); ?>
											...
											<input type="hidden" name="his" size="20" value="<?php echo $_REQUEST['history_id']; ?>">
											...
											<input type="hidden" name="admind" size="20" value="<?php echo $_REQUEST['adateid']; ?>">
											<input type="hidden" name="tdmind" size="20" value="<?php echo $_REQUEST['tdateid']; ?>">
											<input type="hidden" name="sdmind" size="20" value="<?php echo $_REQUEST['sdateid']; ?>">
										</form>
										<!-- END FORM-->
									</div>
								</div>
