		...
		<div class="container">
			...
			<div class="row">
				<div class="col-md-12">
					...
					<h4><a href="home" class="btn blue"> Active <b>(No. <?php 
$result = mysql_query("SELECT * FROM new_request WHERE status = 7 AND csr_id = $ID");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
...
echo $numberOfRows; ?>)</b></a>
						   <a href="home-left" class="btn red"> Left <b>(No. <?php 
$result = mysql_query("SELECT * FROM new_request WHERE status = 6 AND csr_id = $ID");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
...
echo $numberOfRows; ?>)</b></a></h4>
						...
						<?php 
// sending query
$result = mysql_query("SELECT * FROM new_request WHERE status = 7 AND csr_id = $ID ORDER BY request_id DESC;");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
									...
									<div class="timeline-body-head-actions">
										<div class="btn-group">
											...
											<ul class="dropdown-menu pull-right" role="menu">
												...
												<li>
													<a class="allocation" href="#allocation-c" data-toggle="modal" data-target="#allocation-c" data-id="<?php echo $arequest_id; ?>" data-name="<?php echo $aname; ?>">Re-Allocate</a>
												</li>
												<li>
													<a href="left-request?req=<?php echo $arequest_id; ?>">Left </a>
												</li>
												<li>
													<a href="add-parent-account?req=<?php echo $arequest_id; ?>&csr=<?php echo $ID; ?>&name=<?php echo $aname; ?>&email=<?php echo $aemail; ?>&phone=<?php echo $aphone; ?>&skype=<?php echo $askype; ?>&city=<?php echo $acity; ?>&csr_id=<?php echo $acsr_id; ?>">Create Account </a>
												</li>
											</ul>
										</div>
									</div>
