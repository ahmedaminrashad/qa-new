...
if ($c == "#81F781"){ 
$result = mysql_query("SELECT `sched`.*,`course`.`course_yrSec`,`Stime`.`stime_s` FROM `sched`,`course`,`Stime`  WHERE sched.course_id=course.course_id and sched.stime_s_id=Stime.stime_s_id HAVING teacher_id='$tech'");
			//HAVING course_id='$pCourse'
		if (!$result) 
			{
			die("Query to show fields from table failed");
		...
		If ($numberOfRows == 0){
			...
			{			
			 ...
			 if ($hidden_pday == $d && $hidden_pstime == $t){	
			  		echo '<option value="'.$tr.'">'.$time.' ('.$hidden_pcourse.')</option>';
						}			
			}
	}
 }
