...
$date = date('d-m-Y', time());
	mysql_query("INSERT INTO payment (parent_id, parent_name, fee, email, username, userpass, telephone, mobile, skype, c_id, currency_id, mode_id, m_id, group_id)
	SELECT parent_id, parent_name, fee, email, username, userpass, telephone, mobile, skype, c_id, currency_id, mode_id, m_id, group_id
	FROM account WHERE active = 1 AND payment_cycle = 15 AND invoice_type = 1") or die(mysql_error());  
	if(mysql_query){
