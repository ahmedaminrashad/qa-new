...
function class_monitor_num($var1, $var2, $var3){
    $date_start =$_REQUEST['date1'];
$date_end =$_REQUEST['date2'];
    $result = mysql_query("SELECT history_id FROM class_history WHERE course_id = '$var1' AND teacher_id = '$var2' AND monitor_id = '$var3' AND date_teacher >= '$date_start' AND date_teacher <= '$date_end'");
if (!$result) 
	{
    die("Query to show");
	}
...
<a onclick="goBack()" class="btn default">
						...
						<div class="portlet-body">
							<div id="mytable" class="table-responsive">
								...
								<?php
// sending query
   $result = mysql_query("SELECT * FROM `class_history` WHERE teacher_id = '$tech_id' AND date_teacher >= '$date_start' AND date_teacher <= '$date_end' GROUP BY course_id;");
   $counter = 0;
if (!$result) 
	{
    die("Query to show");
