...
$teacher =$_REQUEST['teacher'];
  $date =$_REQUEST['date'];
  $day =$_REQUEST['day'];
mysql_query("DELETE FROM class_history WHERE status < 19 AND monitor_id = 1 AND date_admin = '$date' AND teacher_id = '$teacher'") or die(mysql_error());
    $result = mysql_query("SELECT * FROM sched WHERE aday_id = '$day' AND teacher_id = '$teacher'") or die(mysql_error());
    if (!$result) 
	{
...
If ($numberOfRows == 0) 
	...
	header('Location: teacher-class-search-results-past?&teacher='.$teacher.'&date='.$date.'&check=2');
?>