...
echo $numberOfRows; ?>)</b></a>
						...
						<div class="portlet-body">
							<div id="mytable" class="table-responsive">
								...
								<?php 
$result = mysql_query("SELECT `account`.*, `currency`.`currency_name`, `country`.`c_name`, `time_zone`.* FROM `account`,`currency`,`country`,`time_zone` WHERE account.currency_id=currency.currency_id and account.c_id=country.c_id and account.timezone=time_zone.tz_id HAVING active = 17 ORDER BY parent_id DESC;");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
