...
function pen_task()
  	{$ID = $_SESSION['is']['teacher_id'];
  		$result = mysql_query("SELECT * FROM task WHERE status = 1 and (manager = '$ID' OR user_id = '$ID')");
$counter = 0;
if (!$result) 
	{
...
function com_task()
  	{$ID = $_SESSION['is']['teacher_id'];
  		$result = mysql_query("SELECT * FROM task WHERE status = 2 and (manager = '$ID' OR user_id = '$ID')");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
							...
							<div class="portlet-body todo-project-list-content">
								<div class="todo-project-list">
									<ul class="nav nav-pills nav-stacked">
										<li class="active">
											<a href="pending-task">
											<span class="badge badge-success badge-active"> <?php echo pen_task(); ?> </span> PENDING </a>
										</li>
										<li>
											<a href="completed-task">
											<span class="badge badge-success"> <?php echo com_task(); ?> </span> COMPLETED </a>
										</li>
									</ul>
								</div>
							</div>
						...
						<div class="portlet light">
							...
							<div class="portlet-title">
								<div class="caption">
									...
									<span class="caption-helper">Tasks:</span> &nbsp; <span class="caption-subject font-green-sharp bold uppercase">PENDING (<?php echo pen_task(); ?>)</span>
								</div>
							</div>
							<!-- end PROJECT HEAD -->
...
?>
													...
													<div class="todo-tasklist-item-title">
														<?php if ($pid > 0){echo '<font color="#44B6AE">(Family Name: <a href="parent-accounts-profile?parent_id='.base64_encode($pid).'">'.$pname.'</a>)';} else {echo '';} ?> <font color="#A80707">Assigned to <?php echo $_SESSION['is']['teacher_name']; ?></font>
													</div>
