...
If ($tnumberOfRows == 0){
   ...
   $pnid =base64_decode($_GET["parent_id"]);
	$result = mysql_query("SELECT `account`.*, `country`.`c_a`, `time_zone`.* FROM `account`, `country`, `time_zone` where account.c_id=country.c_id and account.timezone=time_zone.tz_id HAVING parent_id = '$pnid'");
	if (!$result) 
		{
		die("Query to show fields from table failed");
		}
