	...
	<div class="page-head">
		<div class="container">
			...
			<div class="page-title">
				<h1>Teachers<small> In-Active</small></h1>
...
?>
					...
					<h4><?php 
$result = mysql_query("SELECT * FROM profile WHERE active = 2");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
...
$numberOfRows = MYSQL_NUMROWS($result);
echo "Total Number of In-Active Teachers: $numberOfRows"; ?></h4>
...
$result = mysql_query("SELECT `profile`.*, `Gender`.`gender`, `shift`.`shift_name`, `hello`.`inout_name`, `employee_catagory`.`cat_name` FROM
  			`profile`,`Gender`,`shift`,`hello`,`employee_catagory` WHERE profile.g_id=Gender.g_id and profile.shift_id=shift.shift_id and profile.inout_id=hello.inout_id and profile.cat_id=employee_catagory.cat_id HAVING active = 2 and cat_id = 8");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
...
?>
								...
								<td><a href="teacher-delete?pteacher=<?php echo $profile_no; ?>" class="btn btn-icon-only red" onclick="return confirm('<?php echo "Are you sure about deletion of Teacher ". $tname. " ?"; ?>');">
															<i class="fa fa-minus"></i></a></td>
								<td><a href="activate-teacher?pteacher=<?php echo $profile_no; ?>" class="btn btn-icon-only blue" onclick="return confirm('<?php echo "Are you sure about activation of Teacher ". $tname. " ?"; ?>');">
