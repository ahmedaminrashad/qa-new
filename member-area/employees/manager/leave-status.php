							...
							<div class="portlet-body todo-project-list-content">
								<div class="todo-project-list">
									<ul class="nav nav-pills nav-stacked">
										<li class="active">
											<a href="leave-status">
											<span class="badge badge-success badge-active"> <?php echo LeaveApproved1("$tt"); ?> </span> PENDING </a>
										</li>
										<li>
										...
										<li>
											...
											<span class="badge badge-success"> <?php echo LeaveApproved("$tt", "6"); ?> </span> NOT-APPROVED </a>
										</li>
									</ul>
								</div>
							</div>
						...
						<div class="portlet light">
							...
							<div class="portlet-title">
								<div class="caption">
									...
									<span class="caption-helper">Pending:</span> &nbsp; <span class="caption-subject font-green-sharp bold uppercase"> APPLICATIONS (<?php echo LeaveApproved1("$tt"); ?>)</span>
								</div>
							</div>
							<!-- end PROJECT HEAD -->
								...
								<div class="row">
									<div class="">
										<div class="scroller" style="max-height: 800px;" data-always-visible="0" data-rail-visible="0" data-handle-color="#dae3e7">
											...
											<?php 
// sending query
$result = mysql_query("SELECT * FROM leave_app WHERE manager_id = $ID AND (status = 1 OR status = 2 OR status = 3 OR status = 4) ORDER BY leave_id DESC;");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
...
?>
													...
													<?php if ($status == 1){echo '<a class="leave" href="#leave" data-toggle="modal" data-target="#leave" leave-id="'.$leaveid.'" status="2"><button type="button" class="btn green btn-xs">Recommend</button></a> <a class="leave" href="#leave" data-toggle="modal" data-target="#leave" leave-id="'.$leaveid.'" status="6"><button type="button" class="btn red btn-xs">Reject</button></a>'; }
													elseif ($status == 2){echo '<span class="label label-sm label-info">Responce Waiting From Admin</span>'; }
													elseif ($status == 3){echo '<a href="update-leave?status=5&leave='.$leaveid.'"><button type="button" class="btn green btn-xs">Proceed Approval</button>'; }
													elseif ($status == 4){echo '<a href="update-leave?status=6&leave='.$leaveid.'"><button type="button" class="btn red btn-xs">Proceed Rejection</button>'; }?>
														</div>
												</div>
												<?php 	
		$i++;		
