...
$result = mysql_query("SELECT `course`.*, `dept`.`department`, `Gender`.`gender`, `country`.`c_name`, `account`.`parent_name`, `lan`.`lan_name` FROM
  			`course`,`dept`,`Gender`,`country`,`account`,`lan` WHERE course.dept_id=dept.dept_id and course.g_id=Gender.g_id and course.c_id=country.c_id and course.parent_id=account.parent_id and course.lan_id=lan.lan_id HAVING nature = 2 AND m_id = $ID ORDER BY course_id ASC;");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
...
?>
								...
								<td><a href="student-delete?Course_ID=<?php echo $Course_ID; ?>" onclick="return confirm('<?php echo "Are you sure about deletion of student ". $course_yrSec. " ?"; ?>');">
															<button type="button" class="btn red btn-xs"><i class="fa fa-bomb"></i></button></a></td>
							</tr>
							<?php 	
		$i++;		
		}
