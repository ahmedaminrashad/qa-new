...
{
// sending query
   $result = mysql_query("SELECT * FROM test_results Where course_id = $r and m_id = $mon and y_id = $yyy and status_id = 2");
if (!$result) 
	{
    die("Query to show fields from table failed");
	}
		...
		<div class="container">
			...
			<div class="row">
				<div class="col-md-12">
					...
					<h4><?php 
$result = mysql_query("SELECT * FROM course WHERE nature = 1 AND m_id = $ID");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
...
$result = mysql_query("SELECT * FROM course WHERE nature = 2 AND m_id = $ID");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
...
$result = mysql_query("SELECT `course`.*, `dept`.`department`, `Gender`.`gender`, `profile`.`teacher_name`, `country`.`c_name`, `account`.`parent_name`, `lan`.`lan_name` FROM
  			`course`,`dept`,`Gender`,`profile`,`country`,`account`,`lan` WHERE course.dept_id=dept.dept_id and course.g_id=Gender.g_id and course.Teacher=profile.teacher_id and course.c_id=country.c_id and course.parent_id=account.parent_id and course.lan_id=lan.lan_id HAVING nature = 1 AND m_id = '$ID' ORDER BY course_id ASC;");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
...
?>
								...
								<td><a href="re-add-schedule-local?cou_id=<?php echo $Course_ID; ?>"><button type="button" class="btn yellow btn-xs"><i class="fa fa-recycle"></i></button></a></td>
								<td>
