...
$sy = date('Y-m-d');
function attendence($t, $d)
{
// sending query
   $result = mysql_query("SELECT * FROM teacher_attendance Where teacher_id = '$t' and date = '$d'");
if (!$result) 
	{
    die("Query to show fields from table failed");
	}
$tnumberOfRows = MYSQL_NUMROWS($result);
If ($tnumberOfRows == 0){
			echo '<a href="mark-attendence?t_id='.$t.'&status=1"><button type="button" class="btn green btn-xs">P</button></a> <a href="mark-attendence?t_id='.$t.'&status=2"><button type="button" class="btn red btn-xs">A</button></a>';
			}
		else if ($tnumberOfRows > 0) 
			{
			$i=0;
			while ($i<$tnumberOfRows)
				{			
					$tech_att_id = MYSQL_RESULT($result,$i,"attendence_id");
					$tech_status = MYSQL_RESULT($result,$i,"status");
			 if ($tech_status == 1){	
