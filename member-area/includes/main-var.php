...
// Include dbconnection.php - it will return early if already loaded, but $conn should be global
require(__DIR__ . "/dbconnection.php");
require_once(__DIR__ . "/mysql-compat.php");

// Check database connection - access via $GLOBALS to ensure we get the global variable
if (!isset($GLOBALS['conn']) || !($GLOBALS['conn'] instanceof mysqli)) {
    die("Database connection failed. Please contact the administrator. Error: \$conn not available in global scope.");
}
// Make $conn available in local scope
global $conn;
$conn = $GLOBALS['conn'];
