...
$result = mysql_query("SELECT * FROM account Where parent_id = '$var'");
		...
		If ($numberOfRows == 0){
			...
			while ($i<$numberOfRows)
				{			
					$modeid = MYSQL_RESULT($result,$i,"mode_id");
...
If ($numberOfRows == 0) 
	...
	while ($i<$numberOfRows)
		{		
				...
				if($en_name =='11') 
				{
					$level ='<span class="label label-warning"><strong>On Trial</strong></span>';
				}
			else if($en_name =='7')
				{
					$level ='<span class="label label-info"><strong>On Leave</strong></span>';
				}		
			else if($en_name =='3')
				{
					$level ='<span class="label label-danger"><strong>Deactived</strong></span>';
				}
			else if($en_name =='17')
				{
					$level ='<span class="label label-danger"><strong>Suspended</strong></span>';
				}
			else
				{
					$level ='<span class="label label-success"><strong>Regular</strong></span>';
				}
				echo $level;
$i++;		
		}
...
echo $pidm; ?> - 
<?php if ($s == 1){echo 'PENDING';} else {echo 'PAID';}
?></small></h1>
			</div>
			<!-- END PAGE TITLE -->
			<!-- BEGIN PAGE TOOLBAR -->
...
$numberOfRows = MYSQL_NUMROWS($result);
$pidy = MYSQL_RESULT($result,$i,"school_year");
echo $pidy ?> (<?php if ($s == 1){echo 'PENDING';} else {echo 'PAID';}
?>)
</b></font></h4>
...
?>)</a>
															<a href="invoice-received?mon=<?php echo $m; ?>&year=<?php echo $y; ?>&cur=<?php echo $cuid; ?>&currency_name=<?php echo $cname; ?>" class="btn green"><i class="fa fa-user"></i>Total Paid (<?php
// sending query
...
$tnumberOfRows = MYSQL_NUMROWS($result);
echo $tnumberOfRows; ?> -- <?php
$sql = "select sum(fee) from invoice where status = 1 and mon_id = $m and y_id = $y and currency_id = $cuid";
$q = mysql_query($sql);
$row = mysql_fetch_array($q);
$fee = $row[0];
$sql = "select sum(invoice_add) from invoice where status = 1 and mon_id = $m and y_id = $y and currency_id = $cuid";
$q = mysql_query($sql);
$row = mysql_fetch_array($q);
$addfee = $row[0];
$finalFee = $fee+$addfee;
...
?>)</a>
						...
						<div class="portlet-body">
							<div id="mytable" class="table-responsive">
								...
								<?php 
// sending query
$result = mysql_query("SELECT `invoice`.*, `month`.`month_name`, `statusp`.`name`, `country`.`c_name`, `currency`.`currency_name` FROM `invoice`,`month`,`statusp`,`country`,`currency` WHERE invoice.mon_id=month.month_id and invoice.status=statusp.s_id and invoice.c_id=country.c_id and invoice.currency_id=currency.currency_id HAVING mon_id = $m and status = $s and y_id = $y and currency_id = $cuid ORDER BY py_id DESC");
if (!$result) 
	{
    die("Query to show fields from table failed");
	}
...
?>
								...
								<td>
									<a href="paid.php?payment_id=<?php echo $pid; ?>" onclick="return confirm('<?php echo "You are about to mark this invoice paid. Please confirm your action."; ?>');"><?php if ($s == 1){echo '<button type="button" class="btn red btn-xs">Mark</button>'; }?></a><?php if ($s == 2){ echo ''; } ?>
								</td>
								<td><?php echo family_status("$prid"); ?></td>
								<td><a href="<?php echo modeid("$prid") ?>?invoice_no=<?php echo $pid; ?>&account=<?php echo abv("$cuanid"); ?>&month=<?php echo $mon; ?>&year1=<?php echo $pidy; ?>&curnn=<?php echo $cuan; ?>&amount=<?php echo $fe+$fe_add; ?>&parent=<?php echo $prid; ?>&link=<?php echo $link; ?>"><button type="button" title="Email Direct Payment Link" class="btn yellow btn-xs"><i class="fa fa-envelope"></i></button></a></td>
							</tr>
							<?php 	
		$i++;		
		}
