					...
					<h4>List of Accounts for Search Results <b><?php echo $pdept;	?></b></h4>
						<div class="portlet-body">
							<div id="mytable" class="table-responsive">
								...
								<?php 
$result = mysql_query("SELECT `account`.*, `currency`.`currency_name`, `time_zone`.* FROM `account`,`currency`,`time_zone` WHERE account.currency_id=currency.currency_id and account.timezone=time_zone.tz_id HAVING `parent_name` LIKE  '%".$pdept."%' OR `skype` LIKE  '%".$pdept."%' OR `city` LIKE  '%".$pdept."%' OR `email` LIKE  '%".$pdept."%' OR `telephone` LIKE  '%".$pdept."%' OR `mobile` LIKE  '%".$pdept."%'");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
