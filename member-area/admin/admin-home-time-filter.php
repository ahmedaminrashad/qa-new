	...
	<div class="page-head">
		<div class="container">
			...
			<div class="page-title">
				<h1>Admin Home<small> Time Filter</small></h1>
			</div>
			<!-- END PAGE TITLE -->
			<!-- BEGIN PAGE TOOLBAR -->
...
echo $lnumberOfRows; ?>
				...
				<?php 
// sending query
$result1 = mysql_query("SELECT * FROM timestart HAVING time_s_id = $ppdept");
if (!$result1) 
		{
		die("Query to shoiled");
			...
			$teacher = MYSQL_RESULT($result1,$i,"time_s");
					echo "<div class='alert alert-info'>
						<p>
							 Details of classes scheduled at <strong>$teacher</strong>
						</p>
					</div>";
				}
					...
					<!-- END NOTIFICATION DROPDOWN -->
						...
						<div class="portlet-body">
							<div id="mytable" class="table-responsive">
								...
								<?php
// sending query
   $result = mysql_query("SELECT `sched3`.*, `day`.`day_name`, `course`.`course_yrSec`, `timestart`.`time_s`, `3StimeS1time`.`stime_s1`, `3StimeS2time`.`stime_s2`, `monitor`.`mnt_name`,`profile`.`teacher_name`, `dept`.`department` FROM `sched3`,`course`,`profile`,`timestart`,`3StimeS1time`,`3StimeS2time`,`day`,`dept`,`monitor` WHERE sched3.course_id=course.course_id and sched3.teacher_id=profile.teacher_id and sched3.atime_s_id=timestart.time_s_id and sched3.stime_s_id=3StimeS1time.stime_s_id1 and sched3.time_s_id=3StimeS2time.stime_s_id2 and sched3.day_id=day.day_id and sched3.dept_id=dept.dept_id and sched3.mnt_id=monitor.mnt_id HAVING aday_id = $c and atime_s_id = $ppdept ORDER BY atime_s_id;
  			");
if (!$result) 
	{
    die("Query to show fields from table failed");
...
?>
								...
								<td>
									 <a href="history_course_<?php echo $pdt; ?>?Course_ID=<?php echo base64_encode($pCourse); ?>">History</a>
								</td>
								<td>
								...
								<td>
								<?php if ($stua == 11){ echo '<span class="label label-sm label-warning">On Trial</span>'; } elseif ($stua == 17){ echo '<span class="label label-sm label-danger">Suspended</span>'; } else { echo '<span class="label label-sm label-success">Regular</span>'; } ?>
								</td>
