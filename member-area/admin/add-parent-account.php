...
if ($conflict != true){
			mysql_query ("INSERT INTO account(parent_name, email, username, userpass, c_id, telephone, mobile, skype, fee, city, m_id, currency_id, mode_id, date, belong, timezone, trial_date, csr_id, invoice_type, payment_cycle, group_id)
					VALUES('$pname','$pemail','$puser','$ppass','$pcon','$ptele','$pmob','$pskype','$pfee','$pcity','$pman','$pcur','$pmod','$pd','$bl','$tz','$triald','0','$ainvoice','$acycle','$agroup')") or die(mysql_error()); 
				if(mysql_query){
				mysql_query("UPDATE new_request SET status = 5, csr_id = 0 WHERE request_id = '$areq'");
 		header(
