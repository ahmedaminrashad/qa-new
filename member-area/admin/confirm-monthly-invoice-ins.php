...
$hidden_pdept= $_POST['hidden_pdept'];
			 header(
			 		"Location: iup-ins?month=". $_POST['mon']
			 		."&due=". $_POST['dd']
			 		."&year=". $_POST['y']
			 		."&pid=". $_POST['T1']
			 		."&mont=". $_POST['T2']
						...
						<div class="tab-content">
								<div class="portlet box green">
									<div class="portlet-title">
										<div class="caption">
											<i class="fa fa-plus"></i>Please confirm instant invoice for <?php $result = mysql_query("SELECT * FROM account WHERE parent_id = $pid1");
if (!$result) 
	{
    die("Query to show fields from table failed");
	}
$numberOfRows = MYSQL_NUMROWS($result);
$pidm = MYSQL_RESULT($result,$i,"parent_name");
echo $pidm ?>
										</div>
									</div>
	...
	echo $pd;?>" readonly>
												...
												<div class="form-group">
													<label class="col-md-3 control-label"><strong>Fee</strong></label>
													<div class="col-md-4">
														<input type="text" name="fr" id="fr" class="form-control input-circle" value="<?php $result = mysql_query("SELECT `payment`.*, `currency`.`currency_name` FROM `payment`,`currency` WHERE payment.currency_id=currency.currency_id HAVING py_id = 1");
				if (!$result) 
	{
    die("Query to show fields from table failed");
	}
	$pdi = MYSQL_RESULT($result,$i,"fee");
	$cuan = MYSQL_RESULT($result,$i,"currency_name"); ?><?php echo $cuan; ?> <?php echo $pdi; ?>" readonly>
													</div>
												</div>
											...
											<div class="form-actions">
												<div class="row">
													<div class="col-md-offset-3 col-md-9">
														...
														<a onclick="goBack()"><button type="button" class="btn btn-circle default">Cancel</button></a>
													</div>
												</div>
											</div>
										</form>
