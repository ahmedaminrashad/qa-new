...
if ($Adiff < 0 && $admin_startTime > $tech_startTime) {$a = $day-1; if ($a == 0) {$studentDay = 7;} else {$studentDay = $a;}}
if ($Adiff > 0 && $admin_startTime < $tech_startTime) {$a = $day+1; if ($a == 8) {$studentDay = 1;} else {$studentDay = $a;}}
...
mysql_query( "UPDATE sched SET day_id = '$day', sday_id = '$studentDay', aday_id = '$adminDay', time_start = '$tech_startTime', time_end = '$tech_startEnd', start_time_S = '$stud_startTime', end_time_S = '$stud_startEnd', start_time_A = '$admin_startTime', end_time_A = '$admin_startEnd', duration = '$atdif' WHERE sched_id = '$famID'") or die(mysql_error());
