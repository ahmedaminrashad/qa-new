...
<a href="parent-accounts-deactive" class="btn red"><i class="fa fa-user-times"></i> Deactivated (Left, Completed, Deactivated &amp; On Leave) <b>(No. <?php echo NUM("3"); ?>)</b></a>
						...
						<div class="tabbable tabbable-custom tabbable-custom-profile">
							...
							<div id="mytable" class="table-responsive">
								...
								<?php 
$result = mysql_query("SELECT `account`.*, `currency`.`currency_name`, `time_zone`.* FROM `account`,`currency`,`time_zone` WHERE account.currency_id=currency.currency_id and account.timezone=time_zone.tz_id HAVING active = 1 ORDER BY regular_date DESC;");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
...
?>
							...
							<div id="mytable" class="table-responsive">
								...
								<?php 
$result = mysql_query("SELECT `account`.*, `currency`.`currency_name`, `time_zone`.* FROM `account`,`currency`,`time_zone` WHERE account.currency_id=currency.currency_id and account.timezone=time_zone.tz_id HAVING active = 11");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
