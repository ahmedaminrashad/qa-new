...
$student_day1 = $tday1+$a;
			if ($student_day1 == 0){ $sday1 = 7; }
			if ($student_day1 == 8){ $sday1 = 1; }
			else { $sday1 = $student_day1; }
$student_day2 = $tday2+$a;
			if ($student_day2 == 0){ $sday2 = 7; }
			if ($student_day2 == 8){ $sday2 = 1; }
			else { $sday2 = $student_day2; }
$student_day3 = $tday3+$a;
			if ($student_day3 == 0){ $sday3 = 7; }
			if ($student_day3 == 8){ $sday3 = 1; }
			else { $sday3 = $student_day3; }
$student_day4 = $tday4+$a;
			if ($student_day4 == 0){ $sday4 = 7; }
			if ($student_day4 == 8){ $sday4 = 1; }
			else { $sday4 = $student_day4; }
$student_day5 = $tday5+$a;
			if ($student_day5 == 0){ $sday5 = 7; }
			if ($student_day5 == 8){ $sday5 = 1; }
			else { $sday5 = $student_day5; }
$student_day6 = $tday6+$a;
			if ($student_day6 == 0){ $sday6 = 7; }
			if ($student_day6 == 8){ $sday6 = 1; }
			else { $sday6 = $student_day6; }
$student_day7 = $tday7+$a;
			if ($student_day7 == 0){ $sday7 = 7; }
			if ($student_day7 == 8){ $sday7 = 1; }
...
$db = new mysqli($server_db,$username_db,$userpass_db,$name_db);
  $query = "SELECT * FROM profile WHERE active = 1 and cc_id = 1";
  ...
  while($row = $result->fetch_assoc()){
    $subcats2[$row['Teacher']][] = array("id" => $row['course_id'], "val" => $row['course_yrSec']);
  }

  $jsonCats = json_encode($categories);
  $jsonSubCats = json_encode($subcats);
  $jsonSubCats2 = json_encode($subcats2);
?>
<?php
...
<head>
<meta charset="utf-8"/>
<title>TarteeleQuran | Online Learning portel</title>
...
<script type='text/javascript'>
      <?php
        ...
        echo "var subcats2 = $jsonSubCats2; \n";
      ?>
      ...
      function updateSubCats(){
        ...
        for(var i = 0; i < subcats2[Teacher].length; i++){
          subcatSelect2.options[i] = new Option(subcats2[Teacher][i].val,subcats2[Teacher][i].id);
        }
      }
    </script>
<style type="text/css">
...
$numberOfRowsot7 = MYSQL_NUMROWS($result);
If ($numberOfRowsot7 == 0) 
	{
	echo '0';
	}
else if ($numberOfRowsot7 > 0) 
	{
	echo $numberOfRowsot7;
	}
 ?> families</strong> on leave</h3>
					...
					<li class="dropdown dropdown-extended dropdown-dark dropdown-inbox" id="header_inbox_bar">
						...
						<ul class="dropdown-menu">
							<li class="external">
								<h3>You have <strong>7 New</strong> Complaints</h3>
					...
					<li class="dropdown dropdown-user dropdown-dark">
						...
						<span class="username username-hide-mobile">Kiani</span>
						</a>
									...
									<div class="portlet-body form">
										...
										<form action="<?php echo $_SERVER['PHP_SELF']?>" method="post" class="form-horizontal form-row-seperated">
											...
											<div class="form-group">
															...
															<strong>Student</strong></label>
															<div class="col-md-9">
															<select class="form-control" name="pstudent" id="subcatsSelect2"></select>
															</div>
												</div>
										<div class="form-group">
															<label class="control-label col-md-3">
