...
if (!$result)
		...
		If ($numberOfRows == 0){
			...
			while ($i<$numberOfRows)
				{			
					$hidden_pcourse = MYSQL_RESULT($result,$i,"teacher_id");
					$hidden_proom = MYSQL_RESULT($result,$i,"teacher_name");
					echo $hidden_proom;
	...
	<div class="page-head">
		<div class="container">
			...
			<div class="page-title">
				<h1>Complaints <small>Cleared</small></h1>
			</div>
			<!-- END PAGE TITLE -->
		</div>
	</div>
								...
								<div class="todo-project-list">
									<ul class="nav nav-pills nav-stacked">
										<li>
											...
											<span class="badge badge-success"> <?php echo pen_com(); ?> </span> PENDING </a>
										</li>
										<li class="active">
											<a href="complaint-completed">
											<span class="badge badge-success badge-active"> <?php echo com_com(); ?> </span> CLEARED </a>
										</li>
									</ul>
								</div>
							</div>
						...
						<div class="portlet light">
							...
							<div class="portlet-title">
								<div class="caption">
									...
									<span class="caption-helper">Complaints:</span> &nbsp; <span class="caption-subject font-green-sharp bold uppercase">CLEARED (<?php echo com_com(); ?>)</span>
								</div>
							</div>
							<!-- end PROJECT HEAD -->
								...
								<div class="row">
									<div class="">
										<div class="scroller" style="max-height: 800px;" data-always-visible="0" data-rail-visible="0" data-handle-color="#dae3e7">
											...
											<?php 
// sending query
$result = mysql_query("SELECT * FROM complaint WHERE cn_id = '2' ORDER BY com_id DESC;");
$counter = 0;
if (!$result) 
	{
    die("Query to show fields from table failed");
...
If ($numberOfRows == 0) 
	...
	while ($i<$numberOfRows)
		...
		if(($i%2)==0) 
			...
			$res = MYSQL_RESULT($result,$i,"feed");
			$res_date = MYSQL_RESULT($result,$i,"feed_date");
			$res_time = MYSQL_RESULT($result,$i,"feed_time");
			$res_id = MYSQL_RESULT($result,$i,"feed_id");
...
?>
													...
													<div class="todo-tasklist-item-text">
														 <strong>COMPLAINT:</strong> <?php echo $complaint; ?><br><span class="todo-tasklist-date"><i class="fa fa-calendar"></i> <?php echo ''.$dateos.' &nbsp;<i class="fa fa-clock-o"></i>'.$timeos.''; ?> </span>
													</div>
													<div class="todo-tasklist-item-text">
														 <strong><?php echo teacher("$res_id"); ?>:</strong> <?php echo $res; ?><br><span class="todo-tasklist-date"><i class="fa fa-calendar"></i> <?php echo ''.$res_date.' &nbsp;<i class="fa fa-clock-o"></i>'.$res_time.''; ?> </span>
													</div>
