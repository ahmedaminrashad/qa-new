...
if (!isset($conn) || !$conn) {
  ...
  $mm_id =$_REQUEST['mon'];
  $yy_id =$_REQUEST['yr'];
  $dateObj   = DateTime::createFromFormat('!m', $mm_id);
					...
					<div class="portlet light">
						<div class="portlet-body">
							...
							<div id="mytable" class="table-responsive">
								...
								<tr>
								<th>
									 <div class="label label-info"><?php echo $monthName; ?>-<?php echo $yy_id; ?></div>
								</th>
								...
								<?php 
// sending query
$result = mysql_query("SELECT `teacher_salary`.*, `profile`.`teacher_name` FROM
  			`teacher_salary`,`profile` WHERE teacher_salary.teacher_id=profile.teacher_id HAVING MONTH(date) = '$mm_id' AND YEAR(date) = '$yy_id'");
$counter = 0;
if (!$result) 
	{
...
If ($numberOfRows == 0) 
	{
	echo '<div class="label label-danger">No Salary Record Found for '.$monthName.'-'.$yy_id.'.</div>';
	}
...
?>
								...
								<td>
									 <b><a href="teacher-salary-record?ptec=<?php echo $ttt_id; ?>&name=<?php echo $t_name; ?>"><?php echo $t_name; ?></a></b>
								</td>
								...
								<td class="auto-style2 danger">
									 <a class="tax" href="#tax-d" data-toggle="modal" data-target="#tax-d" data-id="<?php echo $sal_id; ?>" data-amu="<?php echo $paid; ?>" data-tax="<?php echo $t_tax ; ?>"><?php echo $t_tax;?></a>
								</td>
								<td class="auto-style2 danger">
...
<div class="modal fade bs-modal-lg" id="tax-d" tabindex="-1" role="dialog" aria-hidden="true">
								<div class="modal-dialog modal-lg">
									<div class="modal-content">
...
$('.tax').click(function(){
    var famID=$(this).attr('data-id');
    var famTax=$(this).attr('data-tax');
    var famAmu=$(this).attr('data-amu');

    $.ajax({url:"salary-tax-details.php?famID="+famID+"&famTax="+famTax+"&famAmu="+famAmu,cache:false,success:function(result){
        $(".modal-content").html(result);
    }});
});
</script>