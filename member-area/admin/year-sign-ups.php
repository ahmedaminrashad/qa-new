...
$y =$_REQUEST['year_id'];
// Connect to MySQL
$link = mysql_connect( 'localhost', 'tarteele_raj', '5506729*' );
if ( !$link ) {
  die( 'Could not connect: ' . mysql_error() );
}

// Select the data base
$db = mysql_select_db( 'tarteele_ss', $link );
if ( !$db ) {
...
{
$y =$_REQUEST['year_id'];
$result = mysql_query("SELECT * FROM account WHERE MONTH(deactivation_date) = $m AND YEAR(deactivation_date) = $y AND MONTH(trial_date) = $m AND YEAR(trial_date) = $y");
...
$result = mysql_query("SELECT * FROM account WHERE MONTH(deactivation_date) = $m AND YEAR(deactivation_date) = $y");
