...
?>)</a>
						...
						<div class="portlet-body">
							<div id="mytable" class="table-responsive">
								...
								<?php 
// sending query
$result = mysql_query("SELECT `invoice`.*, `month`.`month_name`, `statusp`.`name`, `country`.`c_name` FROM `invoice`,`month`,`statusp`,`country` WHERE invoice.mon_id=month.month_id and invoice.status=statusp.s_id and invoice.c_id=country.c_id HAVING mon_id = $m and y_id = $y and currency_id = $cuid and invoice_add != '0' ORDER BY py_id");
if (!$result) 
	{
    die("Query to show fields from table failed");
	}
