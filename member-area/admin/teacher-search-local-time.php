	...
	<div class="page-head">
		<div class="container">
			...
			<div class="page-title">
				<h1>Search Teacher<small> at local time</small></h1>
			</div>
			<!-- END PAGE TITLE -->
			<!-- BEGIN PAGE TOOLBAR -->
	...
	<div class="page-content">
		<div class="container">
			...
			<!-- BEGIN PAGE BREADCRUMB -->
								<?php if (isset($note) && !empty($note)) { echo $note; } ?>
			<ul class="page-breadcrumb breadcrumb">
				<li>
					<a href="admin-home">Home</a><i class="fa fa-circle"></i>
				</li>
				<li class="active">
					 You are Searching Schedule at Your Local-Time
				</li>
			</ul>
			<!-- END PAGE BREADCRUMB -->
			...
			<div class="row">
							...
							<div class="tab-pane active" id="tab_0">
								<div class="portlet box green">
									<div class="portlet-title">
										<div class="caption">
											<i class="fa fa-search"></i>Search Schedule at Your Local-Time
										</div>
										<div class="tools">
									...
									<div class="portlet-body form">
										<!-- BEGIN FORM-->
										<form action="teacher-search-local-time-results" method="post" class="form-horizontal form-row-seperated">
				...
				if ($result) {
                    ...
                    <option value="<?php echo htmlspecialchars($row['TimeID']);?>"><?php echo htmlspecialchars($row['TimeList']);?> </option>
                    <?php }
				} ?>
                  </select>
															</div>
				...
				if ($result) {
				    while ($row = mysql_fetch_assoc($result)) {  ?>
                      <option value="<?php echo htmlspecialchars($row['g_id']);?>"><?php echo htmlspecialchars($row['gender']);?> </option>
                      <?php }
				} ?>
               </select>															
               </div>
				...
				if ($result) {
				    while ($row = mysql_fetch_assoc($result)) {  ?>
                      <option value="<?php echo htmlspecialchars($row['inout_id']);?>"><?php echo htmlspecialchars($row['inout_name']);?> </option>
                      <?php }
				} ?>
               </select>															
               </div>
											...
											<input type="hidden" name="pteacher" size="20" value="<?php echo isset($pT) ? htmlspecialchars($pT) : ''; ?>">
										</form>
										<!-- END FORM-->
									</div>
								</div>
