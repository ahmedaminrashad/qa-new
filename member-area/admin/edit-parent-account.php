   ...
   $pnid =base64_decode($_GET["parent_id"]);
	...
	if (!$result) 
		...
		die("Query to show fields from table failed");
			...
			$status = MYSQL_RESULT($result,$i,"send_status");
			}
?>
<?php
...
?></b></font></h3>
				...
				do {  ?>
                      <option value="<?php echo $row['currency_id'];?>" <?php if($pman == $row['currency_id']){echo 'selected';}?>><?php echo $row['currency_name'];?> </option>
                      <?php } while ($row = mysql_fetch_assoc($result)); ?>
               </select>
															</div>
														</div>
													...
													<div class='col-md-6'>
													    <div class="form-group">
															<label class="control-label col-md-3">
															Notifictions Settings</label>
															<div class="col-md-9">
    															<select required class="form-control" name="status"  >
                                                                    <option value='1' <?php if($status == '1'){echo 'selected';}?>>Email</option>
                                                                    <option value='2' <?php if($status == '2'){echo 'selected';}?>>Whatsapp</option>
                                                                </select>
															</div>
														</div>
													</div>
