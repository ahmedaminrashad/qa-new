...
$time_start = date(" g:i:s", time(true));
...
<?php
// sending query
   $result = mysql_query("SELECT `sched3`.*, `day`.`day_name`, `course`.`course_yrSec`, `timestart`.`time_s`, `3StimeS1time`.`stime_s1`, `3StimeS2time`.`stime_s2`, `monitor`.`mnt_name`,`profile`.`teacher_name`, `dept`.`department` FROM `sched3`,`course`,`profile`,`timestart`,`3StimeS1time`,`3StimeS2time`,`day`,`dept`,`monitor` WHERE sched3.course_id=course.course_id and sched3.teacher_id=profile.teacher_id and sched3.atime_s_id=timestart.time_s_id and sched3.stime_s_id=3StimeS1time.stime_s_id1 and sched3.time_s_id=3StimeS2time.stime_s_id2 and sched3.day_id=day.day_id and sched3.dept_id=dept.dept_id and sched3.mnt_id=monitor.mnt_id HAVING aday_id = $c and atime_s_id = $cc ORDER BY atime_s_id;
  			");
if (!$result) 
	{
    die("Query to show fields from table failed");
...
?>
								...
								<td><span class="label label-sm label-success" title="Taken Regular"><?php echo classes("$pCourse", "$teacher_id", "$mm_id", "$yy_id", "1"); ?></span>-<span class="label label-sm label-info" title="Leave"><?php echo classes("$pCourse", "$teacher_id", "$mm_id", "$yy_id", "6"); ?></span>-<span class="label label-sm label-danger" title="Absent"><?php echo classes("$pCourse", "$teacher_id", "$mm_id", "$yy_id", "4"); ?></span>-<span class="label label-sm label-warning" title="Pending"><?php echo classes("$pCourse", "$teacher_id", "$mm_id", "$yy_id", "9"); ?></span>-<span class="label label-sm label-success" title="Classes/Week"><?php echo schedule_classes($pCourse); ?></span></td>
								<td>
