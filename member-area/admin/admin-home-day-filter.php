...
<?php
  ...
  include("header.php");
  $ppdept =base64_decode($_GET["time_id"]);
	...
	<div class="page-head">
		<div class="container">
			...
			<div class="page-title">
				<h1>Admin Home<small> Class records of <?php 
$result = mysql_query("SELECT * FROM day WHERE day_id = $ppdept");
$counter = 0;
if (!$result) 
	{
...
If ($numberOfRows == 0) 
	...
	{
	$i=0;
			$daddy = MYSQL_RESULT($result,$i,"day_name");
			echo $daddy;
				}
	...
	$i=0;
			$daddy = MYSQL_RESULT($result,$i,"day_name");
					echo "<div class='alert alert-info'>
						<p>
							 You are at class records of <strong>$daddy</strong>
						</p>
					</div>";
				}
					...
					<!-- END NOTIFICATION DROPDOWN -->
						...
						<div class="portlet-body">
							<div id="mytable" class="table-responsive">
								...
								<?php
// sending query
   $result = mysql_query("SELECT `sched3`.*, `day`.`day_name`, `course`.`course_yrSec`, `timestart`.`time_s`, `3StimeS1time`.`stime_s1`, `3StimeS2time`.`stime_s2`, `monitor`.`mnt_name`,`profile`.`teacher_name`, `dept`.`department` FROM `sched3`,`course`,`profile`,`timestart`,`3StimeS1time`,`3StimeS2time`,`day`,`dept`,`monitor` WHERE sched3.course_id=course.course_id and sched3.teacher_id=profile.teacher_id and sched3.atime_s_id=timestart.time_s_id and sched3.stime_s_id=3StimeS1time.stime_s_id1 and sched3.time_s_id=3StimeS2time.stime_s_id2 and sched3.day_id=day.day_id and sched3.dept_id=dept.dept_id and sched3.mnt_id=monitor.mnt_id HAVING aday_id = $ppdept ORDER BY atime_s_id;
  			");
if (!$result) 
	{
    die("Query to show fields from table failed");
