   ...
   if (isset($_POST['cmdSubmit'])) 
{
			...
			mysql_query ("INSERT INTO course(course_yrSec, dept_id, Skype_ID, Age, g_id, c_id, Status, Number_of_Days, Fee, Teacher, Trial_Class, remark_teacher, parent_id, m_id, active, tz_id, timezone, time_name,date,php_tz, lan_id, adept_id, csr_id)
					VALUES('$course','$department','$skype','$age','$gender','$con','$status','$nod','$fee','$teacher','" . mysql_real_escape_string($trialp) . "','" . mysql_real_escape_string($trialt) . "','$p_id','$pmanp','$strial','$tzp','$tzdif','$tzname','$tdate','$tzphp','$langs','$aadepartment','$pcsr')") or die(mysql_error());
 		header(
			 	"Location: update-fee?parent_id=".$abc."&ttcur=".$pcur."&nnod=".$nod."&aaa=".mysql_insert_id()."&emailid=".$p_id."&emailname=".$course."");
				}
?>
<?php
...
?>
				...
				do {  ?>
                      ...
                      <?php // source 1: http://www.dmxzone.com/showDetail.asp?NewsId=5102&TypeId=25
			  	// source 2: http://localhost/phpmyadmin/index.php?db=mydbase&token=651c0063e511c381c9c82ce1fe9b6854
				$result = mysql_query("SELECT * FROM profile WHERE (cat_id = 8 or teacher_rights = 1) and active = 1 and schedule_status = 1 and training = 1 ORDER BY teacher_id ");			  	
				do {  ?>
				...
				do {  ?>
                      ...
                      <?php } while ($row = mysql_fetch_assoc($result)); ?>
												...
												<input type="hidden" value="<?php echo $csr_id; ?>" class="form-control" placeholder="Number of Classes" name="tcsr" id="tcsr">
