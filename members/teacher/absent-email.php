...
$date = date('D jS F-Y', strtotime($Rdate));
$sql = "SELECT * FROM course Where course_id = '$studentID'";
...
$sql = "SELECT * FROM account Where parent_id = '$parentID'";
$result = mysqli_query($conn, $sql);
$numberOfRows = mysqli_num_rows($result);
		if ($numberOfRows == 0){
			echo '';
...
$mail->Subject = 'Student '.$studentName.' is Absent today.';
                  ...
                  <tr>
                    <td align="center" valign="top" style="padding: 0 20px;">
                      <table width="100%" align="center" cellpadding="0" cellspacing="0" border="0">
                        ...
                        <tr> 
                          <td align="left" valign="top" style="font-family: Arial, sans-serif; font-size: 14px; mso-line-height-rule: exactly; line-height: 22px; font-weight: 400;color: #302f35;">This is an important class notification for the class of Student '.$studentName.' scheduled at '.$time.' on '.$date.'.</a></td> 
                        </tr>
                        <tr> 
                            ...
                            <table width="100%" align="center" cellpadding="0" cellspacing="0" border="0">
                              <tr>
                                <td align="center" valign="top">
                                  <table width="100%" align="center" cellpadding="0" cellspacing="0" border="0">
                                    <tr>
                                      <td align="left" valign="top" style="padding: 0 10px 0 0;font-family: Arial, sans-serif; font-size: 14px; mso-line-height-rule: exactly; line-height: 20px; font-weight: 400;color: #302f35;font-weight: 700; width: 208px;">The teacher has Marked the class for Student '.$studentName.' as absent today because Admin 
<strong>'.$site_nameC.'</strong>
                                	  did not receive any notice from your side at least 15 minutes before the class time. There will be no makeup class or adjustment in the invoice available for this class. We would appreciate and provide you makeup class if you could inform us at least 15 mins before the class next time.</td>
                                    </tr> 
                                  	<!-- #{BeginInfo} -->
                                    <tr> 
                                      <td align="center" valign="top" style="padding: 0 10px 0 0;font-family: Arial, sans-serif; font-size: 14px; mso-line-height-rule: exactly; line-height: 20px; font-weight: 400;color: #302f35;font-weight: 700; width: 208px;"><br><font color="blue">Note: If you have already informed Admin HelpDesk in advance for this leave, please contact us as soon as possible so that we can arrange a makeup class for you.</font></td>
                                    </tr>
                                    <tr> 
                                      <td align="center" valign="top" style="font-family: Arial, sans-serif; font-size: 14px; mso-line-height-rule: exactly; line-height: 20px; font-weight: 400;color: #302f35;"><br><a href="https://www.'.$site_nameS.'/members/accounts/request-dispute?parentID='.base64_encode($parentID).'&parentName='.base64_encode($parentName).'&studentName='.base64_encode($studentName).'&teacherID='.base64_encode($teacherID).'&teacherName='.base64_encode($teacherName).'&date='.base64_encode($Adate).'&studentID='.base64_encode($studentID).'&historyID='.base64_encode($historyID).'" ><button class="button" style="vertical-align:middle"><span><b>REQUEST DISPUTE</b></span></button></a></td>
                                    </tr>
                                  	<!-- #{EndInfo} -->                                    
                                  </table>
                                </td>
...
<strong>Accounts '.$site_nameC.'</strong><br>www.'.$site_nameS.'<br>E-mail:'.$email2.'<br>'.$skype_zoom.'<br>Phone:<br>'.$phone1.''.$phone2.''.$phone3.'   
	...
	$mail->msgHTML($body);
    if (!$mail->send()) {
        header('Location: 01-check-sub?parent_id='.$parentID.'');
    } else {
        header('Location: 01-check-sub?parent_id='.$parentID.'');
    }
...
$msg = "*CLASS NOTIFICATION*\r\n\r\n*Student Name:* ".$studentName."\r\nhe teacher has Marked the class for Student ".$studentName." as absent today because Admin ".$site_nameC." did not receive any notice from your side at least 15 minutes before the class time. There will be no makeup class or adjustment in the invoice available for this class. We would appreciate and provide you makeup class if you could inform us at least 15 mins before the class next time.\r\n\r\n*Note: If you have already informed Admin HelpDesk in advance for this leave, please click on the link below to file a dispute so that we can arrange a makeup class for you.*\r\n\r\nhttps://www.".$site_nameS."/members/accounts/request-dispute?parentID=".base64_encode($parentID)."&parentName=".base64_encode($parentName)."&studentName=".base64_encode($studentName)."&teacherID=".base64_encode($teacherID)."&teacherName=".base64_encode($teacherName)."&date=".base64_encode($Adate)."&studentID=".base64_encode($studentID)."&historyID=".base64_encode($historyID)."\r\n\r\n*This is an auto generated message by the system, please do not reply on this number.*";
$postData = array(
