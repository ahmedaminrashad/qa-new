...
$page_subtitle = 'List of in-active Accounts';
                        ...
                        <div class="page-title-wrapper">
                            <div class="page-title-heading">
                                ...
                                <a href="parent-accounts"><button class="mb-2 mr-2 btn btn-shadow btn-success">
                                        Active (Active, Free, Trial &amp; Suspended) <b>(No. <?php
...
echo $numberOfRows; ?>)</b>
                                    ...
                                    <a href="parent-accounts-in-active"><button class="mb-2 mr-2 btn btn-shadow btn-danger">
                                        Deactivated (Left, Completed, Deactivated &amp; On Leave) <b>(No. <?php $sql = "SELECT * FROM account WHERE active = 3 OR active = 7";
$counter = 0;
$result = mysqli_query($conn, $sql);
$numberOfRows = mysqli_num_rows($result);
		if ($numberOfRows == 0){
                    ...
                    <div class="row">
                        ...
                        <div class="mb-3 card">
								...
								<th>
                                 ...
                                 <tbody>
                                 <?php 
$sql = "SELECT `account`.*, `currency`.`currency_name`, `time_zone`.* FROM `account`,`currency`,`time_zone` WHERE account.currency_id=currency.currency_id and account.timezone=time_zone.tz_id HAVING active = 3 ORDER BY regular_date DESC;";
$counter = 0;
$result = mysqli_query($conn, $sql);
$numberOfRows = mysqli_num_rows($result);
		if ($numberOfRows == 0){
...
?>
								...
								<td>
									 <b><?php echo $pname; ?> (<?php echo abcs("$pid"); ?>) <a class="reff" href="#ref" data-toggle="modal" data-target="#ref" data-id="<?php echo $pid; ?>" data-dis="<?php echo $pname; ?>"><?php echo reff("$pid"); ?></a> <?php echo abc3("$pid"); ?></b>
								</td>
								<td>
...
?>
								...
								<th>
									 &nbsp;
                                            ...
                                            <tbody>
                                            <?php 
$sql = "SELECT `account`.*, `currency`.`currency_name`, `time_zone`.* FROM `account`,`currency`,`time_zone` WHERE account.currency_id=currency.currency_id and account.timezone=time_zone.tz_id HAVING active = 7 ORDER BY parent_id DESC;";
$counter = 0;
$result = mysqli_query($conn, $sql);
$numberOfRows = mysqli_num_rows($result);
		if ($numberOfRows == 0){
