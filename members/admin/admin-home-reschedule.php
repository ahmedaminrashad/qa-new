...
$numberOfRows = mysqli_num_rows($result);
		...
		elseif ($numberOfRows > 0) 
			...
			while ($row = mysqli_fetch_assoc($result)){			
					...
					echo '<div class="ml-auto badge badge-danger">'.$hidden_tt.'</div>';
			}
			}
}
$sy = date('Y-m-d');
...
$page_subtitle = 'List of all classes pending approval!';
$table_name = 'CLasses';
?>
...
$new_date = date('l jS F-Y', $old_date_timestamp); echo '<div class="ml-auto badge badge-success">'.$new_date.'</div>'; ?> <div class="ml-auto badge badge-success">Pending Approval</div></h5>
								...
								<th>
									 &nbsp;
                                            ...
                                            <?php 
// sending query
$sql = "SELECT * FROM `class_resched` ORDER BY start_time_A";
$counter = 0;
$result = mysqli_query($conn, $sql);
$numberOfRows = mysqli_num_rows($result);
if ($numberOfRows == 0) 
...
elseif ($numberOfRows > 0) 
	...
	while($row = mysqli_fetch_array($result))
		{		
			...
			$his_ledate = $row['le_date_admin'];
			$his_ori = $row['ori_status'];
			$his_cur = $row['cur_status'];
?>
								...
								<td><a href="approve-reschedule?his_id=<?php echo $sched_id; ?>&original_id=<?php echo $uni_id; ?>"><div class="ml-auto badge badge-success">Yes</div></a></td>
								<td><?php if ($his_cur == 1) { echo '<a href="not-reschedule?his_id='.$sched_id.'&original_id='.$uni_id.'"><div class="ml-auto badge badge-danger">No</div></a>'; } elseif ($his_cur == 2) { echo '<a href="not-advance?his_id='.$sched_id.'&original_id='.$uni_id.'"><div class="ml-auto badge badge-danger">No</div></a>'; }?></td>
