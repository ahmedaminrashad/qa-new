...
<div class="app-main__outer">
            ...
            <!-- App inner start-->
                ...
                <!-- Page Title Start-->
					...
					$ccc =$_REQUEST['cour'];
					$ddd =$_REQUEST['adate'];
$sql = "SELECT `class_history`.*,`course`.`course_yrSec`,`monitor`.`mnt_name`,`remaks`.`remak`,`profile`.`teacher_name` FROM `class_history`,`course`,`monitor`,`remaks`,`profile` WHERE class_history.course_id=course.course_id and class_history.monitor_id=monitor.mnt_id and class_history.teacher_remarks=remaks.remk_id and class_history.teacher_id=profile.teacher_id HAVING course_id = '$ccc' AND date_admin = '$ddd'";
$result = mysqli_query($conn, $sql);
$numberOfRows = mysqli_num_rows($result);
		if ($numberOfRows == 0){
			echo '';
...
?>
			...
			<?php if ($restatus == 1) { echo '<a href="history-delete?history_id='.$hid.'"><button class="mb-2 mr-2 btn btn-outline-danger">Delete</button></a> <a class="allocation1" href="#allocation-c" data-toggle="modal" data-target="#allocation-c" data-id="'.$hid.'"><button class="mb-2 mr-2 btn btn-outline-info">Re-Schedule</button></a> <a href="leave_absent_mark?history_id='.$hid.'&attend=5"><button class="mb-2 mr-2 btn btn-outline-warning">Leave</button></a>'; } ?></b></font>
									</h4>
