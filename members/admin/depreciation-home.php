                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="main-card mb-3 card">
								...
								<?php 
// sending query
$sql = "SELECT `account_entry`.*, `accounts_head`.`account_head_name`, `vendor`.`vendor_name` FROM `account_entry`,`accounts_head`, `vendor` WHERE account_entry.account_head=accounts_head.account_head_id and account_entry.vendor_id=vendor.vendor_id HAVING (ac_cat_id = 10 OR ac_cat_id = 1) AND status = 1 ORDER BY date DESC";
$result = mysqli_query($conn, $sql);
$numberOfRows = mysqli_num_rows($result);
		if ($numberOfRows == 0){
			echo '';
...
?>
								<tr bgcolor="<?php if ($vmod == 1) { echo '#FFF0A9'; } else { echo '#CFFDA8'; } ?>">
								<td>
								...
								<td>
									<a href="add-adjustment-entry?en_id=<?php echo $en_id; ?>&en_head=<?php echo $en_head; ?>&ac_c_id=<?php echo $vmod; ?>&link=<?php echo $link; ?>"><button class="mb-2 mr-2 btn btn-success"><i class="fa fa-plus"></i></button></a>
								</td>
							</tr>
						<?php 	
		}
...
<?php } ?>
            ...
            <div class="modal-footer">
                ...
                <button type="submit" name ="submit" class="btn btn-primary">Serach</button>
            </div>
            </form>
        </div>
    </div>
