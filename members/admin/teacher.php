...
$new_date = date('l jS F-Y', $old_date_timestamp); echo '<div class="ml-auto badge badge-success">'.$new_date.'</div>'; ?> <div class="ml-auto badge badge-success">Admin Home</div></h5>
								...
								<?php
// sending query
   $sql = "SELECT * FROM `class_history` WHERE date_admin = '$data_date' AND teacher_id = '$tech_id' ORDER BY start_time_A";
$result = mysqli_query($conn, $sql);
...
?>
								...
								<td>
									 <a class="allocation1" href="#allocation-c" data-toggle="modal" data-target="#allocation-c" data-id="<?php echo $sched_id; ?>"><?php echo date("g:i a", strtotime($teacher_timeS)); ?> - <?php echo date("g:i a", strtotime($teacher_timeE)); ?></a>
								</td>
								<td>
				...
				$set_icon= '<i class="fa fa-clock-o"></i>';
								...
								<td>
									<a class="allocation" href="#allocation-c" data-toggle="modal" data-target="#allocation-c" data-id="<?php echo $sched_id; ?>" data-name="<?php echo StudentName("$student_id"); ?>" data-time="<?php echo time1("$teacher_time"); ?>"><button class="mb-2 mr-2 btn btn-outline-danger">Shift</button></a>
								</td>
								...
								<td><a class="invoicelink" href="#invoice" data-toggle="modal" data-target="#invoice" data-id="<?php echo $sched_id; ?>" data-amu="<?php echo $teacher_id; ?>"><button class="mb-2 mr-2 btn btn-outline-warning">Link</button></a></td>
								<td><a href="valid.php?parent_id=<?php echo $sched_id; ?>"><button class="mb-2 mr-2 btn btn-outline-warning"><i class='fa fa-history'></i></button></a></td>
							</tr>
							<?php 	
		}
	}	
