...
$result = mysqli_query($mysql, "SELECT `invoice`.*, `currency`.`currency_name` FROM `invoice`,`currency` WHERE invoice.currency_id=currency.currency_id HAVING py_id = $payID");
...
if ($row['whats_email'] == 'email'){
 //This iterator syntax only works in PHP 5.4+
    $mail->addAddress($ppemail, $ppname);
    $time_start = date(" g:i:A", time(true));
                                ...
                                <td align="center" valign="top">
                                  <table width="100%" align="center" cellpadding="0" cellspacing="0" border="0">
                                    ...
                                    <tr> 
                                      ...
                                      <td align="left" valign="top" style="font-family: Arial, sans-serif; font-size: 14px; mso-line-height-rule: exactly; line-height: 20px; font-weight: 400;color: #302f35;">'.$dateF.'</td>
                                    </tr>
                                  	<!-- #{EndInfo} -->                                    
                                  </table>
                                </td>
