...
//echo '<br>DUR '.$atdif;
$sql = "INSERT INTO class_history(course_id, teacher_id, date_teacher, date_student, date_admin, parent_id, dept_id, adept_id, status, time_start, time_end, start_time_S, end_time_S, start_time_A, end_time_A, duration, type, le_date_admin, le_date_teacher, le_date_student)
					VALUES('$SID','$teacher_name','$date','$studentDay','$adminDay','$PID','$dept','$adept', '20', '$tech_startTime', '$tech_startEnd', '$stud_startTime', '$stud_startEnd', '$admin_startTime', '$admin_startEnd', '$atdif', '11', '$ADtate', '$TDtate', '$SDtate')";
					if ($conn->query($sql) === TRUE) {
					echo ''; }
					require ("../includes/dbconnection.php");
					$sql = "UPDATE class_history SET re_status = 2, re_date_admin = '$adminDay', re_date_student = '$studentDay', re_date_teacher = '$date' WHERE history_id = $famID";
if ($conn->query($sql) === TRUE) { header('Location: ' . $_SERVER['HTTP_REFERER']);
}
