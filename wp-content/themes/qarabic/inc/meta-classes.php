...
if( ! class_exists( 'bespecial_Meta_Box' )  ) :
class bespecial_Meta_Box {
	/**
	...
	function __construct( $args = null ){
		$this->meta_box_id 		= $args['meta_box_id'] ? : 'bespecial_meta_box';
		$this->label 			= $args['label'] ? : 'bespecial Metabox';
	...
	public function meta_fields_callback() {
		...
		echo '<input type="hidden" name="bespecial_cmb_nonce" id="bespecial_cmb_nonce" value="' .
		...
		foreach ( $this->fields as $field ) {
			...
			elseif( $field['type'] == 'title' ){
				echo $this->field_title( $field );
			}
			do_action( "bespecial_meta_field-{$field['name']}", $field, $post->post_type );
		}
	...
	public function save_meta_fields( $post_id, $post ) {
		if (
			! isset( $_POST['bespecial_cmb_nonce'] ) ||
			! wp_verify_nonce( $_POST['bespecial_cmb_nonce'], plugin_basename( __FILE__ ) ) ||
	...
	public function field_hr(){
		$html	.= '<hr>';
		return $html;
	}
	public function field_title( $field ){
		global $post;
		$class  = isset( $field['class'] ) && ! is_null( $field['class'] ) ? $field['class'] : 'bespecial-meta-field';
		$html	= sprintf( '<fieldset class="bespecial-row" id="bespecial_cmb_fieldset_%1$s">', $field['name'] );
		$html	.= sprintf( '<h2 class="title">%1$s</h2>', $field['label']);
		$html	.= '</fieldset>';
		return $html;
	}
	public function field_text( $field ){
		...
		$html	= sprintf( '<fieldset class="bespecial-row" id="bespecial_cmb_fieldset_%1$s">', $field['name'] );
		$html	.= sprintf( '<label class="bespecial-label" for="bespecial_cmb_%1$s">%2$s</label>', $field['name'], $field['label']);
		$html  .= sprintf( '<input type="%1$s" class="%2$s" id="bespecial_cmb_%3$s" name="%3$s" value="%5$s" %6$s %7$s/>', $field['type'], $class, $field['name'], $field['name'], $value, $readonly, $disabled );
	...
	public function field_textarea( $field ){
		...
		$html	= sprintf( '<fieldset class="bespecial-row" id="bespecial_cmb_fieldset_%1$s">', $field['name'] );
		$html	.= sprintf( '<label class="bespecial-label" for="bespecial_cmb_%1$s">%2$s</label>', $field['name'], $field['label']);
		$html  .= sprintf( '<textarea rows="' . $rows . '" cols="' . $cols . '" class="%1$s-text" id="bespecial_cmb_%2$s" name="%3$s" %4$s %5$s >%6$s</textarea>', $class, $field['name'], $field['name'], $readonly, $disabled, $value );
		$html .= $this->field_description( $field );
		$html	.= '</fieldset>';
		return $html;
	}
	public function field_radio( $field ){
		...
		$disabled  = isset( $field['disabled'] ) && ( $field['disabled'] == true ) ? " disabled" : "";
        $html	= sprintf( '<fieldset class="bespecial-row" id="bespecial_cmb_fieldset_%1$s">', $field['name'] );
        $html .= '<label class="bespecial-label">'.$field['label'].'</label>';
	...
	public function field_checkbox( $field ){
		...
		$html	= sprintf( '<fieldset class="bespecial-row" id="bespecial_cmb_fieldset_%1$s">', $field['name'] );
		$html	.= sprintf( '<label class="bespecial-label" for="bespecial_cmb_%1$s">%2$s</label>', $field['name'], $field['label']);
		$html  .= sprintf( '<input type="checkbox" class="checkbox" id="bespecial_cmb_%1$s" name="%1$s" value="on" %2$s %3$s />', $field['name'], checked( $value, 'on', false ), $disabled );
	...
	public function field_select( $field ){
		...
		$html	= sprintf( '<fieldset class="bespecial-row" id="bespecial_cmb_fieldset_%1$s">', $field['name'] );
        $html	.= sprintf( '<label class="bespecial-label" for="bespecial_cmb_%1$s">%2$s</label>', $field['name'], $field['label']);
        $html   .= sprintf( '<select class="%1$s" name="%2$s" id="bespecial_cmb_%2$s" %3$s %4$s>', $class, $name, $disabled, $multiple );
        if( $multiple == '' ) :
        foreach ( $field['options'] as $key => $label ) {
	...
	public function field_color( $field ){
		...
		$html	= sprintf( '<fieldset class="bespecial-row" id="bespecial_cmb_fieldset_%1$s">', $field['name'] );
		$html	.= sprintf( '<label class="bespecial-label" for="bespecial_cmb_%1$s">%2$s</label>', $field['name'], $field['label']);
        $html  .= sprintf( '<input type="text" class="%1$s-text wp-color-picker-field" id="bespecial_cmb_%2$s" name="%2$s" value="%4$s" data-default-color="%5$s" />', $class, $field['name'], $field['name'], $value, $field['default'] );
		$html	.= $this->field_description( $field );
		$html	.= '</fieldset>';
        return $html;
	}
	public function field_file( $field ){
        ...
        $html	= sprintf( '<fieldset class="bespecial-row" id="bespecial_cmb_fieldset_%1$s">', $field['name'] );
        $html	.= sprintf( '<label class="bespecial-label" for="bespecial_cmb_%1$s">%2$s</label>', $field['name'], $field['label']);
        $html  .= sprintf( '<input type="text" class="%1$s-text bespecial-file" id="bespecial_cmb_%2$s" name="%2$s" value="%3$s" %4$s />', $class, $field['name'], $value, $disabled );
        $html  .= '<input type="button" class="button bespecial-browse" data-title="' . $field['label'] . '" data-select-text="' . $select_button . '" value="' . $upload_button . '" ' . $disabled . ' />';
        $html  .= $this->field_description( $field );
        $html	.= '</fieldset>';
        return $html;
	}
	public function field_wysiwyg( $field ){
		...
		$class  = isset( $field['class'] ) && ! is_null( $field['class'] ) ? $field['class'] : 'bespecial-meta-field';
		$width  = isset( $field['width'] ) && ! is_null( $field['width'] ) ? $field['width'] : '100%';
	...
	public function field_description( $args ) {
        if ( ! empty( $args['desc'] ) ) {
        	if( isset( $args['desc_nop'] ) && $args['desc_nop'] ) {
        		$desc = sprintf( '<small class="bespecial-small">%s</small>', $args['desc'] );
        	} else{
    ...
    function scripts() {
        ...
        <script>
            jQuery(document).ready(function($) {
                ...
                // media uploader
                $('.bespecial-browse').on('click', function (event) {
                    ...
                    file_frame.on('select', function () {
                        ...
                        self.prev('.bespecial-file').val(attachment.url);
        ...
        <style type="text/css">
            ...
            .bespecial-row { border-bottom: 1px solid #ebebeb; padding: 8px 4px; }
            .bespecial-row:last-child { border-bottom: 0px;}
            .bespecial-row .bespecial-label {display: inline-block; vertical-align: top; width: 200px; font-size: 15px; line-height: 24px;}
            .bespecial-row .bespecial-browse { width: 96px;}
            .bespecial-row .bespecial-file { width: calc( 100% - 110px ); margin-right: 4px; line-height: 20px;}
            #postbox-container-1 .bespecial-meta-field, #postbox-container-1 .bespecial-meta-field-text {width: 100%;}
            #postbox-container-2 .bespecial-meta-field, #postbox-container-2 .bespecial-meta-field-text {width: 74%;}
            #postbox-container-1 .bespecial-meta-field-text.bespecial-file { width: calc(100% - 101px) }
            #postbox-container-2 .bespecial-meta-field-text.bespecial-file { width: calc(100% - 306px) }
            #wpbody-content .metabox-holder { padding-top: 5px; }
						hr {display:block;height:15px;border-top: 5px solid #ebebeb;margin-top: -3px;}
						.bespecial-row h2 {font-size:16px !important;font-weight:900 !important;background:#ebebeb;}
        </style>
        <?php
    }
}
endif;
if ( ! function_exists( 'bespecial_meta_box' ) ) {
	function bespecial_meta_box( $args ){
		return new \bespecial_Meta_Box( $args );
	}
}
